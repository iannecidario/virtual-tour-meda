...
export function hotspotToMarker(hotspot, selectedId = null) {
  ...
  return {
    ...
    html: `
      <button class="hotspot-marker${selectedClass}${navigationClass}${searchClass}" type="button" data-hotspot-id="${escapeHtml(hotspot.id)}" style="--hotspot-size: ${size}px; --hotspot-icon-size: ${iconSize}px; --hotspot-color: ${escapeHtml(hotspot.color)}; --hotspot-border-color: ${escapeHtml(hotspot.borderColor || '#FFFFFF')}" aria-label="${label}">
        ${iconMarkup}
      </button>
    `,
